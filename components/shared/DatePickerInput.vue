          ...
          <div class="bg-slate-800 border border-slate-600 rounded-lg shadow-2xl p-4 w-96">
            ...
            <div class="flex items-center justify-between mb-4">
              <button 
                type="button" 
                @click="changeMonth(-1)" 
                @keydown="(e) => { if (e.key === 'Enter') changeMonth(-1) }"
                ...
                <div class="w-[160px]">
                  <div class="relative" data-dropdown="month">
                    <button
                      ...
                      @keydown="(e) => { if (e.key === 'Enter') toggleMonthDropdown() }"
                    ...
                    >
                      <button
                        ...
                        :class="{ 
                          'bg-orange-600 hover:bg-orange-700': selectedMonth === option.value,
                          'bg-slate-600': highlightedMonthIndex === index && selectedMonth !== option.value
                        }"
                      >
                        {{ option.label }}
                      </button>
                ...
                <div class="w-[120px]">
                  <div class="relative" data-dropdown="year">
                    <button
                      ...
                      @keydown="(e) => { if (e.key === 'Enter') toggleYearDropdown() }"
                      class="w-full h-8 pl-3 pr-2 bg-slate-800 border border-slate-600 rounded text-white text-sm focus:outline-none focus:ring-2 focus:ring-orange-600 focus:border-transparent transition-all duration-200 hover:border-slate-500 text-left flex items-center justify-between"
                      tabindex="0"
                    >
                    ...
                    >
                      <button
                        ...
                        class="w-full px-3 py-2 text-left hover:bg-slate-700 focus:bg-slate-700 focus:outline-none transition-colors duration-150 text-white text-sm"
                        :class="{ 
                          'bg-orange-600 hover:bg-orange-700': selectedYear === option.value,
                          'bg-slate-600': highlightedYearIndex === index && selectedYear !== option.value
                        }"
                      >
                        {{ option.label }}
                      </button>
            ...
            <div class="grid grid-cols-7 gap-1">
              <button 
                ...
                :class="[
                  ...
                  highlightedDateIndex === index && !date.isSelected && !date.isFuture ? 'bg-slate-600 ring-2 ring-orange-500' : ''
                ]"
              >{{ date.day }}</button>
            </div>
          </div>
...
const highlightedMonthIndex = ref(-1)
const highlightedYearIndex = ref(-1)
const highlightedDateIndex = ref(-1)
...
const handleKeyDown = (event: KeyboardEvent) => {
  ...
  // Handle Enter key to open date picker
  if (key === 'Enter') {
...
const handleCalendarKeyDown = (event: KeyboardEvent) => {
  ...
  const currentIndex = highlightedDateIndex.value
  ...
  if (event.key === 'ArrowDown') {
    ...
    if (currentIndex < dates.length - 1) {
      highlightedDateIndex.value = currentIndex + 1
    } else {
      highlightedDateIndex.value = 0 // Wrap to first
    }
    return
  }
  ...
  if (event.key === 'ArrowUp') {
    ...
    if (currentIndex > 0) {
      ...
      highlightedDateIndex.value = dates.length - 1 // Wrap to last
    }
    return
  }
  ...
  // Handle Enter key to select highlighted date
  if (event.key === 'Enter') {
    event.preventDefault()
    event.stopPropagation()
...
const openDatePicker = () => {
  ...
  nextTick(() => {
    ...
    // Add keyboard listener for calendar navigation
...
const handleMonthKeyDown = (event: KeyboardEvent) => {
  ...
  if (event.key === 'ArrowDown') {
    ...
    if (highlightedMonthIndex.value < monthOptions.value.length - 1) {
      highlightedMonthIndex.value++
    } else {
      highlightedMonthIndex.value = 0 // Wrap to first
    }
    
    // Scroll to highlighted option
    const optionElement = document.querySelector(`[data-month-option="${monthOptions.value[highlightedMonthIndex.value].value}"]`) as HTMLElement
    if (optionElement) {
  ...
  if (event.key === 'ArrowUp') {
    ...
    if (highlightedMonthIndex.value > 0) {
      highlightedMonthIndex.value--
    } else {
      highlightedMonthIndex.value = monthOptions.value.length - 1 // Wrap to last
    }
  ...
  if (event.key === 'Enter') {
    ...
    if (highlightedMonthIndex.value >= 0) {
      ...
      selectMonth(monthOptions.value[highlightedMonthIndex.value].value)
    } else if (monthSearchQuery.value) {
  ...
  if (key.length === 1 && /[a-z0-9]/i.test(key)) {
    ...
    const matchingIndex = monthOptions.value.findIndex(option => 
      option.label.toLowerCase().startsWith(monthSearchQuery.value.toLowerCase())
    )
    
    if (matchingIndex >= 0) {
      highlightedMonthIndex.value = matchingIndex
      // Scroll to the matching option
      const optionElement = document.querySelector(`[data-month-option="${monthOptions.value[matchingIndex].value}"]`) as HTMLElement
      if (optionElement) {
        optionElement.scrollIntoView({ block: 'nearest' })
      }
    }
...
const handleYearKeyDown = (event: KeyboardEvent) => {
  ...
  if (event.key === 'ArrowDown') {
    ...
    if (highlightedYearIndex.value < yearOptions.value.length - 1) {
      highlightedYearIndex.value++
    } else {
      highlightedYearIndex.value = 0 // Wrap to first
    }
    
    // Scroll to highlighted option
    const optionElement = document.querySelector(`[data-year-option="${yearOptions.value[highlightedYearIndex.value].value}"]`) as HTMLElement
    if (optionElement) {
      optionElement.scrollIntoView({ block: 'nearest' })
    }
    return
  ...
  if (event.key === 'ArrowUp') {
    ...
    if (highlightedYearIndex.value > 0) {
      highlightedYearIndex.value--
    } else {
      highlightedYearIndex.value = yearOptions.value.length - 1 // Wrap to last
    }
  ...
  if (event.key === 'Enter') {
    ...
    if (highlightedYearIndex.value >= 0) {
      // Select highlighted option
      selectYear(yearOptions.value[highlightedYearIndex.value].value)
    } else if (yearSearchQuery.value) {
      // Select first matching option from search
  ...
  if (key.length === 1 && /[a-z0-9]/i.test(key)) {
    ...
    // Find first option that starts with the search query
    const matchingIndex = yearOptions.value.findIndex(option => 
      option.label.toLowerCase().startsWith(yearSearchQuery.value.toLowerCase())
    )
    
    if (matchingIndex >= 0) {
      highlightedYearIndex.value = matchingIndex
      // Scroll to the matching option
      const optionElement = document.querySelector(`[data-year-option="${yearOptions.value[matchingIndex].value}"]`) as HTMLElement
      if (optionElement) {
        optionElement.scrollIntoView({ block: 'nearest' })
      }
    }
